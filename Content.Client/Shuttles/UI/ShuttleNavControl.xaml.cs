...
public sealed partial class ShuttleNavControl : BaseShuttleControl
{
    ...
    private record struct ShuttleCalculatePositionsJob : IParallelRobustJob
    {
        ...
        public IResourceCache resCache;
        ...
        public void Execute(int index)
        {
            ...
            if (!gridData.ContainsKey(gUid))
            {
                ...
                if (iconQuery.TryComp(gUid, out var vesselIcon)
                    && vesselIcon.Icon != null
                    && TryResolveVesselIcon(vesselIcon.Icon, spritesSys, resCache) is { } vesselTexture)
                {
                    data.icon = vesselTexture;
    ...
    /// <summary>
    ///     Crescent - resolves a vessel's radar icon, or null when the sprite it names is not there.
    /// </summary>
    /// <remarks>
    ///     A grid carries its icon as map data, so a renamed or deleted RSI state outlives every map that
    ///     still points at it. Asking the sprite system for one of those hands back the error texture and
    ///     writes a log line per grid, so the console ends up drawing red error boxes over a wreck field
    ///     and spamming the client log while it does it. Checking first leaves the caller with no icon,
    ///     which the radar already draws as a plain contact dot.
    /// </remarks>
    private static Texture? TryResolveVesselIcon(SpriteSpecifier icon, SpriteSystem sprites, IResourceCache cache)
    {
        switch (icon)
        {
            case SpriteSpecifier.Rsi rsi:
                if (!cache.TryGetResource<RSIResource>(SpriteSystem.TextureRoot / rsi.RsiPath, out var res)
                    || !res.RSI.TryGetState(rsi.RsiState, out _))
                {
                    return null;
                }
            ...
            case SpriteSpecifier.Texture tex:
                if (!cache.TryGetResource<TextureResource>(SpriteSystem.TextureRoot / tex.TexturePath, out _))
                    return null;
